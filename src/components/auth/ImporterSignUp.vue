<template>
    <v-app class="tw-p-8 tw-bg-white tw-rounded-2xl tw-max-w-7xl tw-mx-auto tw-mt-10">
        <div class="tw-flex tw-flex-col md:tw-flex-row tw-gap-8">
            ...
            <div class="tw-flex-1 tw-flex tw-flex-col tw-gap-4">
                <h2 class="tw-text-lg tw-font-semibold tw-text-indigo-600">Business Details</h2>
                <v-text-field
                    label="Business Name"
                    dense
            ...
            <!-- Password Section -->
            <div class="tw-flex-1 tw-flex tw-flex-col tw-gap-4">
                <h2 class="tw-text-lg tw-font-semibold tw-text-indigo-600">Password</h2>
                <v-text-field
                    label="Password"
    ...
    async signUpUser() {
      try {
        ...
        const userPayload = {
          ...
          companyAddress: this.form.location.customName,
        };

        const saveResponse = await axios.post('/api/auth/register/importer', userPayload);
        if (saveResponse.data.success === true) {
          this.$emit('importer-registered');
          this.$toast.success('Importer account created successfully! Please sign in.');
        } else {
