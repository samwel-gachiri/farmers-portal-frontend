...
export default {
  ...
  methods: {
    ...
    async loadStatistics() {
      try {
        const response = await this.$axios.get(`/api/v1/aggregators/${this.aggregatorId}/statistics`);
    ...
    async loadCollectionEvents() {
      ...
      try {
        const response = await this.$axios.get(`/api/v1/aggregators/${this.aggregatorId}/collection-events`);
    ...
    async loadConsolidatedBatches() {
      ...
      try {
        const response = await this.$axios.get(`/api/v1/aggregators/${this.aggregatorId}/consolidated-batches`);
    ...
    async submitCollection() {
      ...
      try {
        ...
        await this.$axios.post(`/api/v1/aggregators/${this.aggregatorId}/collection-events`, payload);
    ...
    async markAsPaid(item) {
      try {
        await this.$axios.patch(`/api/v1/aggregators/collection-events/${item.id}/payment`, {
          eventId: item.id,
          paymentStatus: 'PAID',
        });
    ...
    viewBatchDetails(item) {
      // Navigate to batch details
      console.log('View batch:', item);
    },

    async markBatchShipped(item) {
      try {
        await this.$axios.patch(`/api/v1/aggregators/consolidated-batches/${item.id}/status?status=SHIPPED`);
