...
export default {
  ...
  methods: {
    ...
    async loadStatistics() {
      try {
        const response = await this.$axios.get(`/api/v1/importers/${this.importerId}/statistics`);
    ...
    async loadShipments() {
      ...
      try {
        const response = await this.$axios.get(`/api/v1/importers/${this.importerId}/shipments`);
    ...
    uploadDocument(shipment) {
      ...
      console.log('Upload document for:', shipment);
    },

    recordInspection(shipment) {
      // Navigate to inspection form
      console.log('Record inspection for:', shipment);
    },

    updateShipmentStatus(shipment) {
      // Show status update dialog
      console.log('Update status for:', shipment);
    },

    reviewCompliance(shipment) {
      // Navigate to compliance review
      console.log('Review compliance for:', shipment);
    },
